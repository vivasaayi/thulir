...
public class DataSetSyncService
{
    ...
    public async Task<S3SyncResult> WriteFilesToDatabase(List<S3Object> files)
    {
        ...
        foreach (var file in files)
        {
            try
            {
                string command = @"INSERT INTO sourceimages (imageid, s3path, indexedtime, imagename)
                               VALUES (@imageId, @s3Path, @indexedTime, @imagename)";
                ...
                await _dal.InsertRecord(command, new 
                {
                    ...
                    imagename = Utils.FileUtils.GetFileNameFromS3Path(file.Key)
                });
